...
namespace UI
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            {
                Microsoft.AspNetCore.Identity.SignInResult result = await this._signInManager.PasswordSignInAsync(model.Username, model.Password, model.RememberLogin, lockoutOnFailure: true);
                if (result.Succeeded)
                {
                    ApplicationUser user = await this._userManager.FindByNameAsync(model.Username);
        ...
        private async Task<LoginViewModel> BuildLoginViewModelAsync(string returnUrl)
        {
            ...
            {
                Client client = await this._clientStore.FindEnabledClientByIdAsync(context.ClientId);
                if (client != null)
                {
        ...
        private async Task<LogoutViewModel> BuildLogoutViewModelAsync(string logoutId)
        {
            ...
            LogoutRequest context = await this._interaction.GetLogoutContextAsync(logoutId);
