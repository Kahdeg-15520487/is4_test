...
namespace UI
{
    ...
    public class ExternalController : Controller
    {
        ...
        public async Task<IActionResult> Callback()
        {
            ...
            (ApplicationUser user, string provider, string providerUserId, IEnumerable<Claim> claims) = this.FindUserFromExternalProvider(result);
            if (user == null)
            {
                // this might be where you might initiate a custom workflow for user registration
                // in this sample we don't show how that would be done, as our sample implementation
                // simply auto-provisions new external user
                user = AutoProvisionUserAsync(provider, providerUserId, claims).Result;
            }
        ...
        private (ApplicationUser user, string provider, string providerUserId, IEnumerable<Claim> claims) FindUserFromExternalProvider(AuthenticateResult result)
        {
            ...
            Claim userIdClaim = externalUser.FindFirst(JwtClaimTypes.Subject) ??
                              externalUser.FindFirst(type: ClaimTypes.NameIdentifier) ??
            ...
            // find external user
            ApplicationUser user = this._userManager.FindByLoginAsync(provider, providerUserId).Result;
        ...
        private async Task<ApplicationUser> AutoProvisionUserAsync(string provider, string providerUserId, IEnumerable<Claim> claims)
        {
            ...
            IdentityResult identityResult = await this._userManager.CreateAsync(user);
